...
const productSchema = z.object({
  ...
  image: z.string().optional(), // Nova Propriedade para PWA offline storage (Compressão local Base64)
});
...
export function ProductForm({ initialData, onSubmit, categories }: ProductFormProps) {
  ...
  const {
    register,
    handleSubmit,
    setValue, // Usado para injetar código externo via Câmera e Canvas
  ...
  /**
   * Pega o arquivo do input escondido, converte para canvas 400x400 para compressão brutal
   * e extrai como JPEG Base64 (Ideal para IndexedDB - O(1) read sem blobs físicos).
   */
  const handleImageCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      const img = new Image();
      img.onload = () => {
        // Redimensionamento Extremo "Smart Crop" 400x400
        const SIZE = 400;
        ...
        if (ctx) {
          // Calcula Crop centralizado da foto
          ...
          // Extrai Compressão em string para o DB
          const base64Data = canvas.toDataURL("image/jpeg", 0.6); // 60% Qualidade agressiva (-50kb)
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6 w-full">
        ...
        {/* Camada Fotográfica PWA com Capture="environment" */}
        <div className="flex justify-center mb-2">
          <label className="relative w-32 h-32 rounded-3xl overflow-hidden bg-[#20201f] border border-[#484847]/40 flex flex-col items-center justify-center cursor-pointer hover:border-[#53ddfc] transition-colors shadow-sm active:scale-95 group">
            ...
            {/* Input Fantasma Mobile -> Invoca Câmera/Galeria Nativa */}
            <input 
              type="file" 
              accept="image/*" 
              capture="environment" // iOS/Android Lente Traseira Automática
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* Field: Código de Barras c/ Scanner Hook */}
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              ...
              />
              {/* Botão Gatilho do Scanner Visor Cheio */}
              <button 
                type="button" // OBRIGATÓRIO: impede submite falso do react hook form
        ...
        <button 
          ...
          className="w-full bg-[#06B6D4] hover:bg-[#53ddfc] text-[#004b58] font-black text-lg uppercase tracking-wider py-4 rounded-xl mt-4 shrink-0 active:scale-[0.98] transition-transform disabled:opacity-50"
        >
          Salvar Produto
        </button>
      </form>

      {/* Render Condicional do Visor de Lente */}
      {isScannerOpen && (
        <BarcodeScannerModal 
          onClose={() => setScannerOpen(false)}
          onDetected={(code) => {
            setValue("barcode", code, { shouldValidate: true, shouldDirty: true });
            setScannerOpen(false); // Despacha e se auto-destroi liberando a lente
