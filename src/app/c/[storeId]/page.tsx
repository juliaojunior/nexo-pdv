...
export default async function StoreCatalogPage(props: { params: Promise<{ storeId: string }> }) {
  ...
  try {
    ...
    const [pResult, cResult, sResult] = await Promise.all([
      client.sql`SELECT id as local_id, name, price, stock, category_id as categoryid, image_url as imageurl, promotional_price as "promotionalPrice", promotion_end_date as "promotionEndDate", description FROM nexo_products WHERE user_id = ${storeId} ORDER BY name ASC`,
